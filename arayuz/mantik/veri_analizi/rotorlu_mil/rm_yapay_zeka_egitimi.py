...
def yapay_zeka_egit_rotorlu_mil():
    print("Rotorlu Mil eğitimi başlatılıyor...")
    ...
    try:
        ...
        for sayfa in excel_dosyasi.sheet_names:
            if "ROTORLU" in sayfa.upper():
                hedef_sayfa = sayfa
                break

        if not hedef_sayfa:
            print("HATA: İçinde 'ROTORLU' geçen bir sayfa bulunamadı!")
            return
        ...
        print("\n" + "=" * 30)
        print("🎯 YAPAY ZEKA BAŞARI ORANLARI (ROTORLU MİL)")
        ...
        joblib.dump(rf_resim, os.path.join(kayit_klasoru, "rm_resim_model.pkl"))
        joblib.dump(le_resim, os.path.join(kayit_klasoru, "rm_resim_encoder.pkl"))
        joblib.dump(rf_sira, os.path.join(kayit_klasoru, "rm_sira_model.pkl"))
        joblib.dump(le_sira, os.path.join(kayit_klasoru, "rm_sira_encoder.pkl"))
        joblib.dump(encoders, os.path.join(kayit_klasoru, "rm_girdi_encoders.pkl"))

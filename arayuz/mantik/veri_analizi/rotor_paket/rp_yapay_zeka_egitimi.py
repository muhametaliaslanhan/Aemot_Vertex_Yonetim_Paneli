...
def yapay_zeka_egit_rotor_paket():
    print("Rotor Paket eğitimi başlatılıyor...")
    ...
    try:
        ...
        if not hedef_sayfa:
            print("HATA: İçinde 'ROTOR PAKET' geçen bir sayfa bulunamadı!")
            return
        ...
        print("\n" + "=" * 30)
        print("🎯 YAPAY ZEKA BAŞARI ORANLARI (ROTOR PAKET)")
        print(f"Sıra No Öğrenme Oranı  : %{sira_basarisi:.2f}")
        print("=" * 30 + "\n")
        # --------------------------------------------------------

        kayit_klasoru = os.path.dirname(__file__)
        joblib.dump(rf_sira, os.path.join(kayit_klasoru, "rp_sira_model.pkl"))
        joblib.dump(le_sira, os.path.join(kayit_klasoru, "rp_sira_encoder.pkl"))
        joblib.dump(encoders, os.path.join(kayit_klasoru, "rp_girdi_encoders.pkl"))

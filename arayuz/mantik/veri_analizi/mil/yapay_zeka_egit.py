...
def yapay_zeka_egit_mil():
    print("MIL eğitimi başlatılıyor. Excel aranıyor...")
    ...
    try:
        df = pd.read_excel(excel_yolu, sheet_name="MIL", header=7, dtype=str).fillna("")
        ...
        for sutun in girdi_sutunlari + [hedef_sutun_resim, hedef_sutun_sira]:
            if sutun not in df.columns:
                print(f"\nKRİTİK HATA: '{sutun}' sütunu Excel'de bulunamadı!")
                return

        print("Veriler ayıklanıyor ve etiketleniyor...")
        ...
        resim_tahmin = rf_resim.predict(X)
        ...
        print("\n" + "=" * 30)
        print("🎯 YAPAY ZEKA BAŞARI ORANLARI (MIL)")
        ...
        joblib.dump(rf_resim, os.path.join(kayit_klasoru, "mil_resim_model.pkl"))
        joblib.dump(le_resim, os.path.join(kayit_klasoru, "mil_resim_encoder.pkl"))
        joblib.dump(rf_sira, os.path.join(kayit_klasoru, "mil_sira_model.pkl"))
        joblib.dump(le_sira, os.path.join(kayit_klasoru, "mil_sira_encoder.pkl"))
        joblib.dump(encoders, os.path.join(kayit_klasoru, "mil_girdi_encoders.pkl"))

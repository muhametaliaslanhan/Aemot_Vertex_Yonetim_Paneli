...
class SarilmisBobinArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        self.ozel_frekans_kutusu.addItems(
            ["", "33", "50", "60", "67", "75", "87", "100", "106", "120", "133", "135.33"])
        ...
        duzen_opsiyon.addWidget(QLabel("Özel Frekans:"), 0, 2)
        duzen_opsiyon.addWidget(self.ozel_frekans_kutusu, 0, 3)

        duzen_opsiyon.addWidget(QLabel("Özel Voltaj:"), 2, 0)
        ...
        duzen_onaylar.addWidget(self.cb_vantilator, 0, 1)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (SARILIS BOBIN)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "SARILMIS BOBIN"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "SARILMIS BOBIN":
                    hedef_sayfa = sayfa
                    break
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "SARILMIS BOBIN"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                temiz_sayfa_adi = sayfa.strip().upper()
                if "SARILMIS" in temiz_sayfa_adi and "BOBIN" in temiz_sayfa_adi:
        ...
        try:
            ...
            self.tablo.setSortingEnabled(True)
            QMessageBox.warning(self, "Hata", f"Excel dosyasında 'SARILMIS BOBIN' sayfası bulunamadı!")
        except Exception as e:
            self.tablo.setSortingEnabled(True)
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = SarilmisBobinGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if "SARILMIS" in temiz_sayfa and "BOBIN" in temiz_sayfa:
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA SARILMIŞ BOBİN UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki bobin(ler)in AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_sarilmis_bobin.xlsx")
            ...
            # DİKKAT: HÜCRE G8 OLARAK DÜZELTİLDİ
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_grup, taslak_metin):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                if "SARILMIS" in sayfa.upper() and "BOBIN" in sayfa.upper():
                    hedef_sayfa = sayfa
                    break
    ...
    def uret_calistir(self):
        ...
        # --- AÇIKLAMA 2'Yİ OTOMATİK ÜRETMESİ VE TEMİZLİĞİ ---
        ...
        # ---------------- 3. DNA TESTİ İLE SIRA BULMA ----------------
        tam_metin_havuzu = f"{taslak_aciklama} {aciklama2_oto}"

        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="SARILMIS BOBIN", aranan_grup=grup,
                                              taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="SARILMIS BOBIN", id_kodu="SB")
        ...
        # NOT: Açıklama 2 sisteme otomatik üretilen haliyle ekleniyor.
        yeni_kayit = [
...
class SarilmisBobinGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Sarılmış Bobin Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("Sarılmış Bobin Parametreleriyle Filtrele")
        form_layout = QGridLayout()
        # Sınıfın geri kalan kısmına aynen dokunma, bırak kalsın.
        ...
        self.ek_aciklama_input.setPlaceholderText("Ek Açıklama (Örn: ÖZEL FREKANS)")
        ...
        form_layout.addWidget(self.cb_vantilator, 4, 0)
    ...
    def arama_yap(self):
        try:
            ...
            if paket_boyu:
                # Sarılmış Bobin'de PB: takısı yok, doğrudan aranıyor!

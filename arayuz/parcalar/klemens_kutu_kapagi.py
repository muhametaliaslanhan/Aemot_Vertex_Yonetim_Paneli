...
class Klemens_Kutusu_kapagiArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        baslik = QLabel("KLEMENS KUTU VE KAPAĞI")
        ...
        grup_ozel = QGroupBox("Klemens Kutu Kapağı Detayları")
        ...
        self.govde_malzemesi_kutusu.addItems(["ALUMINYUM", "PIK", "PLASTIK", "SAC"])

        self.urun_kutusu = QComboBox()
        self.urun_kutusu.addItems([
            "KLEMENS KUTUSU",
            "KLEMENS KUTUSU KAPAGI",
            "KLEMENS KUTUSU GOVDE KAPAGI",
            "KLEMENS KUTU GOVDE BAGLANTISI",
            "KLEMENS KUTUSU RAKOR KAPAGI",
            "KLEMENS KUTUSU SAC"
        ])
        ...
        duzen_ozel.addWidget(self.govde_malzemesi_kutusu, 1, 3)

        duzen_ozel.addWidget(QLabel("Ürün:"), 2, 0)
        duzen_ozel.addWidget(self.urun_kutusu, 2, 1)
        ...
        # ---------------- TABLO ----------------
        self.tablo = QTableWidget(0, 8)  # DİKKAT: 7 YERİNE 8 OLACAK
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (KLEMENS KUTU VE KAPAĞI)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "KLEMENS KUTU VE KAPAGI"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "KLEMENS KUTU VE KAPAGI":
                    hedef_sayfa = sayfa
                    break
            ...
            for index, row in df.iterrows():
                ...
                if "-KL-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Örn: YMI063090-KL-0001-00-0000 -> 2. İndeks: "0001"
                    if len(parcalar) >= 3:
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "KLEMENS KUTU VE KAPAGI"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                temiz_sayfa_adi = sayfa.strip().upper()
                if temiz_sayfa_adi == "KLEMENS KUTU VE KAPAGI":
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = KlemensGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if temiz_sayfa == "KLEMENS KUTU VE KAPAGI":
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA KLEMENS UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki klemens(ler)in AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_klemens.xlsx")
            ...
            # DİKKAT: Tablomuz artık 8 Sütun! Kayıt tarihi H8'e yazılır.
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_aciklama2):
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama2 == hedef_dna:
                # 8 Sütunlu Klemens tablosunda RESİM NO 4. İndekste olduğu için, SIRA NO 5. İndekstedir!
    ...
    def uret_calistir(self):
        ...
        urun = self.urun_kutusu.currentText().strip()
        ...
        if not urun: bos_alanlar.append("Ürün")
        ...
        # ÖZEL KURAL: İşlenmiş ibaresi ne zaman yazılacak?
        if islenmis == "ISLENMEMIS" or (malzeme == "ALUMINYUM" and urun == "KLEMENS KUTUSU KAPAGI"):
            pass
        else:
            taslak_parcalari.append("ISLENMIS")

        taslak_parcalari.append(urun)
        taslak_parcalari.append(f"- {malzeme}")
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Klemens Ürünü ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        # 1. Grubu baştan sil (Örn: "500" veya "355-400" baştan uçacak)
        if aciklama2_oto.startswith(grup):
            aciklama2_oto = aciklama2_oto[len(grup):].strip()

        # 2. Resim No ve Revizyon Mantığı (Tam attığın örneklere göre)
        if resim_no and resim_no != "0000":
            # Resim no doluysa (örn: 3407), sadece resim no'yu sondan uçur, R:01 kalsın.
            aciklama2_oto = aciklama2_oto.replace(f"- {resim_no}", "").strip()
        else:
            # Resim no 0000 ise, revizyonu (Örn: - R:01) komple sil at.
            aciklama2_oto = re.sub(r'\s*-\s*R:\d+', '', aciklama2_oto).strip()

        # ---------------- 3. DNA TESTİ İLE SIRA NO BULMA ----------------
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="KLEMENS KUTU VE KAPAGI", aranan_aciklama2=aciklama2_oto)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="KLEMENS KUTU VE KAPAGI", id_kodu="KL")
        ...
        temiz_grup = grup.replace("-", "")
        if len(temiz_grup) == 3:
            id_grup = f"{temiz_grup}000"
        else:
            id_grup = temiz_grup
        ...
        uretilen_id = f"YMI{id_grup}-KL-{aciklama_kod_str}-{id_revizyon}-{id_resim}"

        # ---------------- 5. TABLOYA YAZDIRMA ----------------
        bugunun_tarihi = QDateTime.currentDateTime().toString("dd.MM.yyyy HH:mm")

        # DİKKAT: 8 Sütunlu yapı (RESİM NO eklendi)
        yeni_kayit = [
...
class KlemensGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("KLEMENS KUTU VE KAPAGI Gelişmiş Arama")
        ...
        gercek_sayfa = "KLEMENS KUTU VE KAPAGI"
        try:
            dosya = pd.ExcelFile(self.excel_yolu)
            for s in dosya.sheet_names:
                if s.strip().upper() == "KLEMENS KUTU VE KAPAGI":
                    gercek_sayfa = s
                    break
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("KLEMENS Parametreleriyle Filtrele")
        form_layout = QGridLayout()

        # Klemens İçin Gerekli Kutular
        self.versiyon_kutusu = QComboBox()
        self.versiyon_kutusu.addItems(["", "ESKİ_GÖVDE", "PREMIUM"])
        ...
        self.urun_kutusu = QComboBox()
        self.urun_kutusu.addItems(["", "KLEMENS KUTUSU", "KLEMENS KUTUSU KAPAGI", "KLEMENS KUTUSU GOVDE KAPAGI",
                                   "KLEMENS KUTU GOVDE BAGLANTISI", "KLEMENS KUTUSU RAKOR KAPAGI", "KLEMENS KUTUSU SAC"])
        ...
        form_layout.addWidget(self.islenmis_islenmemis_kutusu, 1, 1)
        form_layout.addWidget(QLabel("Malzeme:"), 1, 2)
        form_layout.addWidget(self.govde_malzemesi_kutusu, 1, 3)

        form_layout.addWidget(QLabel("Ürün:"), 2, 0)
        form_layout.addWidget(self.urun_kutusu, 2, 1)
    ...
    def arama_yap(self):
        try:
            ...
            urun = self.urun_kutusu.currentText().strip().upper()
            ...
            if urun:
                urun_temiz = urun.replace(" ", "")
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(urun_temiz, regex=False)]

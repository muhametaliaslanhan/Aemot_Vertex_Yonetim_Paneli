...
class RulmanYagKapagiArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        baslik = QLabel("RULMAN YAĞ KAPAĞI ARAYÜZÜ")
        ...
        grup_ozel = QGroupBox("Rulman Yağ Kapağı Detayları")
        duzen_ozel = QGridLayout()

        self.ic_dis_kutusu = QComboBox()
        self.ic_dis_kutusu.addItems(["IC", "DIS", "IC-DIS"])
        ...
        duzen_ozel.addWidget(QLabel("İç-Dış:"), 0, 0)
        duzen_ozel.addWidget(self.ic_dis_kutusu, 0, 1)

        duzen_ozel.addWidget(QLabel("İşlenmiş/İşlenmemiş:"), 0, 2)
        duzen_ozel.addWidget(self.islenmis_islenmemis_kutusu, 0, 3)

        duzen_ozel.addWidget(QLabel("Ek Açıklama:"), 1, 0)
        duzen_ozel.addWidget(self.ek_aciklama_input, 1, 1, 1, 3)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (RULMAN YAĞ. KAPAĞI)")
    ...
    def formu_temizle(self):
        ...
        for nesne in self.findChildren(QLineEdit):
            nesne.clear()  # Arama kutuları da dahil temizleniyor!
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "RULMAN YAĞ. KAPAĞI"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "RULMAN YAĞ. KAPAĞI":
                    hedef_sayfa = sayfa
                    break
            ...
            for index, row in df.iterrows():
                ...
                if "-RY-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Format: YMI315-RY-0001-00 -> 2. İndeks: "0001"
                    if len(parcalar) >= 3:
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "RULMAN YAĞ. KAPAĞI"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                temiz_sayfa_adi = sayfa.strip().upper()
                if temiz_sayfa_adi == "RULMAN YAĞ. KAPAĞI":
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = RulmanYagKapagiGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if temiz_sayfa == "RULMAN YAĞ. KAPAĞI":
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA RULMAN YAĞ. KAPAĞI UYARISI!",
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_ry.xlsx")
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_aciklama2):
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama2 == hedef_dna:
                # 7 Sütunlu RY tablosunda SIRA NO 4. İndekstedir!
    ...
    def uret_calistir(self):
        ...
        ic_dis = self.ic_dis_kutusu.currentText().strip()
        ...
        if not ic_dis: bos_alanlar.append("İç/Dış")
        ...
        taslak_aciklama += f" {ic_dis} RULMAN YAGLAMA KAPAGI - {islenmis}"

        if versiyon != "ESKİ_GÖVDE":
            taslak_aciklama += " - R:01"
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Yağ Kapağı ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        # Revizyon numarasını sondan sil
        aciklama2_oto = re.sub(r'\s*-\s*R:\d+$', '', aciklama2_oto).strip()

        # ---------------- 3. DNA TESTİ İLE SIRA NO BULMA ----------------
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="RULMAN YAĞ. KAPAĞI", aranan_aciklama2=aciklama2_oto)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="RULMAN YAĞ. KAPAĞI", id_kodu="RY")
        ...
        id_grup = grup[:3]
        id_revizyon = "00" if versiyon == "ESKİ_GÖVDE" else "01"

        uretilen_id = f"YMI{id_grup}-RY-{aciklama_kod_str}-{id_revizyon}"
...
class RulmanYagKapagiGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("RULMAN YAĞ KAPAĞI Gelişmiş Arama")
        ...
        gercek_sayfa = "RULMAN YAĞ. KAPAĞI"
        try:
            dosya = pd.ExcelFile(self.excel_yolu)
            for s in dosya.sheet_names:
                if s.strip().upper() == "RULMAN YAĞ. KAPAĞI":
                    gercek_sayfa = s
                    break
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("Rulman Yağ Kapağı Parametreleriyle Filtrele")
        ...
        form_layout.addWidget(QLabel("İç/Dış:"), 1, 0)
        form_layout.addWidget(self.ic_dis_kutusu, 1, 1)
    ...
    def arama_yap(self):
        try:
            ...
            ic_dis = self.ic_dis_kutusu.currentText().strip().upper()
            ...
            if ic_dis:
                # IC, DIS veya IC-DIS ararken karışmaması için
                ic_dis_temiz = ic_dis.replace("-", "")
                if ic_dis_temiz == "IC":
                    df_filtrelenmis = df_filtrelenmis[
                        metin_sifir_bosluk.str.contains("IC", regex=False) &
                        ~metin_sifir_bosluk.str.contains("ICDIS", regex=False)
                    ]
                elif ic_dis_temiz == "DIS":
                    df_filtrelenmis = df_filtrelenmis[
                        metin_sifir_bosluk.str.contains("DIS", regex=False) &
                        ~metin_sifir_bosluk.str.contains("ICDIS", regex=False)
                    ]
                else:
                    df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(ic_dis_temiz, regex=False)]

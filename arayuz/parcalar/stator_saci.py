...
class StatorSaciArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        self.grup_kutusu.addItems(
            ["063", "071", "080", "090", "100", "112", "132", "160", "180", "200", "225", "250", "280", "315", "355",
             "400", "450", "500", "630","NORDEX","1X25 MW","2MW NRDX"])
        ...
        grup_ozel = QGroupBox("Stator SACI Detayları")
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (STATOR SACI)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "STATOR SACI"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "STATOR SACI":
                    hedef_sayfa = sayfa
                    break
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "STATOR SACI"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                temiz_sayfa_adi = sayfa.strip().upper()
                if "STATOR" in temiz_sayfa_adi and "SACI" in temiz_sayfa_adi:
        ...
        try:
            ...
            self.tablo.setSortingEnabled(True)
            QMessageBox.warning(self, "Hata", f"Excel dosyasında 'STATOR SACI' sayfası bulunamadı!")
        except Exception as e:
            self.tablo.setSortingEnabled(True)
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = StatorSaciGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if "STATOR" in temiz_sayfa and "SACI" in temiz_sayfa:
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA STATOR SACI UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki sac(lar)ın AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_stator_saci.xlsx")
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_grup, taslak_metin):
        def kelime_kumesi_yap(metin):
            ...
            return set([k.strip() for k in temiz_metin.split() if k.strip()])
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                if "STATOR" in sayfa.upper() and "SACI" in sayfa.upper():
                    hedef_sayfa = sayfa
                    break
    ...
    def uret_calistir(self):
        ...
        ek_aciklama = ham_ek_aciklama.replace("STATOR SACI", "").strip()
        ...
        taslak_parcalari = [f"{grup}-{kutup}"]

        if ek_aciklama:
            taslak_parcalari.append(ek_aciklama)

        taslak_parcalari.append("STATOR SACI")
        ...
        # 3. KUTUP TİRESİ KONTROLÜ (Tire yoksa ekle, varsa dokunma!)
        if aciklama2_oto and not aciklama2_oto.startswith("-"):
            aciklama2_oto = f"-{aciklama2_oto}"

        # ---------------- 3. DNA TESTİ İLE SIRA NO BULMA ----------------
        tam_metin_havuzu = f"{taslak_aciklama} {aciklama2_oto}"

        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="STATOR SACI", aranan_grup=grup, taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="STATOR SACI", id_kodu="SS")
        ...
        uretilen_id = f"YMI{grup}-SS-{aciklama_kod_str}-{id_revizyon}"
        ...
        for r in range(self.tablo.rowCount()):
            if self.tablo.item(r, 0) and self.tablo.item(r, 0).text().strip() == uretilen_id:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu Stator Sacı ZATEN LİSTEDE var!\nID: {uretilen_id}\n\nAynı sacı ard arda ekleyemezsiniz.")
                return
...
class StatorSaciGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Stator Sacı Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("Stator Sacı Parametreleriyle Filtrele")
    ...
    def arama_yap(self):
        try:
            ...
            if grup:
                grup_temiz = grup.replace(" ", "")  # Arayüzden geleni boşluksuz yap

                if "NORDEX" in grup_temiz or "NRDX" in grup_temiz:
                    # NOR, DEX, NRDX gibi parçalı yazımları hem Grup sütununda hem tüm metinde arar
                    df_filtrelenmis = df_filtrelenmis[
                        (df_filtrelenmis.iloc[:, 2].astype(str).str.strip().str.upper().isin(
                            [grup, "NORDEX", "NOR", "NRDX", "2MW NRDX"])) |
                        (metin_sifir_bosluk.str.contains(grup_temiz, regex=False))
                        ]
                elif "25MW" in grup_temiz:
                    # 1X2, 5MW gibi parçalı yazımları yakalar
                    df_filtrelenmis = df_filtrelenmis[
                        (df_filtrelenmis.iloc[:, 2].astype(str).str.strip().str.upper().isin([grup, "1X2", "1X25"])) |
                        (metin_sifir_bosluk.str.contains("1X25MW", regex=False))
                        ]
                else:
                    # Standart gruplar (132, 160 vs)
                    df_filtrelenmis = df_filtrelenmis[

...
class GovdeArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        duzen_temel.addWidget(QLabel("Govde Boyu:"), 1, 0)
        duzen_temel.addWidget(self.govde_boyu_kutusu, 1, 1)

        grup_temel.setLayout(duzen_temel)
        form_ana_duzen.addWidget(grup_temel)

        # --- GRUP 2: Rotorlu Mile Özel Girdiler ---
        grup_ozel = QGroupBox("Gövde Detayları")
        ...
        self.islenmis_islenmemis.addItems(["ISLENMIS", "ISLENMEMIS"])
        ...
        duzen_ozel.addWidget(QLabel("Ayaklı/Ayaksız:"), 1, 2)
        duzen_ozel.addWidget(self.ayak_kutusu, 1, 3)
        ...
        # ---------------- TABLO ----------------
        self.tablo = QTableWidget(0, 8)  # DİKKAT: 7 DEĞİL 8 OLACAK!
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (GOVDE)")
    ...
    def tabloda_arama_yap(self):
        ...
        # DÜZELTME 1: "Gövde Ayak" kayıtlarında GOVDE kelimesi geçmez.
        # Kullanıcı alışkanlıkla "gövde" yazarsa aramanın patlamaması için bu kelimeyi gizlice siliyoruz.
        aranan_aciklama = aranan_aciklama.replace("GOVDE", "").strip()
        ...
        try:
            ...
            for index, row in df_veri.iterrows():
                ...
                if id_eslesti and aciklama_eslesti:
                    ...
                    # Hücre kaymasını önleyen asıl kısım!
            ...
            self.tablo.setSortingEnabled(True)
            QMessageBox.warning(self, "Hata", f"Excel dosyasında 'GOVDE' sayfası bulunamadı!")
        except Exception as e:
            self.tablo.setSortingEnabled(True)
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = GovdeGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_govde.xlsx")
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if kelime_kumesi_yap(tablo_grup) == aranan_grup_kumesi:
                ...
                if programin_dna_kumesi == tablo_dna_kumesi:
                    # Tabloda 5. indeks SIRA NO (8 Sütunlu tablo olduğu için)
    ...
    def uret_calistir(self):
        ...
        # ---------------- YENİ: YASAKLI GÖVDE KONTROLÜ ----------------

        try:
            # Json okuma işlemini kendi json_islemleri modülüne de alabilirsin, hızlıca böyle de çözersin
            with open(kaynak_yolu('arayuz/mantik/katalog_veri_json/govde_yasaklari.json'), 'r',
                      encoding='utf-8') as f:
                yasakli_listesi = json.load(f)
        except:
            yasakli_listesi = {}

        # Arayüzden alınan verilerle, Excel'deki formata uygun kontrol stringi oluştur (Örn: 063 ALUMINYUM AYAKLI ISLENMIS R:01)
        # R: kısmını versiyona göre ayarlıyoruz
        rev_metni = f"R:{revizyon}" if versiyon != "ESKİ_GÖVDE" else ""
        kontrol_metni = f"{grup} {malzeme} {ayak} {islenmis} {rev_metni}".upper().strip()
        # Fazla boşlukları yutmak için
        kontrol_metni = " ".join(kontrol_metni.split())

        if kontrol_metni in yasakli_listesi:
            QMessageBox.critical(self, "YASAK KOMBİNASYON",
                                 f"Üretim Hatası: Bu gövde özellikleri bir araya gelemez!\n\nEngellenen Tanım: {kontrol_metni}")
            return  # Fonksiyonu burada keser, kod üretmez, listeye eklemez!
        # --------------------------------------------------------------

        ham_ek_aciklama = turkce_karakter_temizleme(self.ek_aciklama_input.text()).strip().upper()
            # ... (Buradan sonrası senin yazdığın taslak_parcalari falan diye devam ediyor) ...
        ek_aciklama = ham_ek_aciklama.replace("DOKUM GOVDE", "").strip()
        ...
        if not ayak: bos_alanlar.append("Ayaklı/Ayaksız")
        ...
        # Eğer kullanıcı Resim No boşken "Evet yine de ekle" dediyse, sistem patlamasın diye 0000 yapıyoruz
        if not resim_no:
            resim_no = "0000"
        ...
        taslak_parcalari.append("DOKUM GOVDE")
        ...
        taslak_aciklama = " ".join(taslak_aciklama.split())  # Fazla boşlukları yut
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="GOVDE", aranan_grup=grup, taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, taslak_aciklama, sayfa_adi="GOVDE", id_kodu="GV")
        ...
        uretilen_id = f"YMI{grup}-GV-{aciklama_kod_str}-{id_revizyon}-{id_resim}"
        ...
        for r in range(self.tablo.rowCount()):
            if self.tablo.item(r, 0) and self.tablo.item(r, 0).text().strip() == uretilen_id:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu Gövde ZATEN LİSTEDE var!\nID: {uretilen_id}\n\nAynı gövdeyi ard arda ekleyemezsiniz.")
                return

        # ---------------- 5. TABLOYA YAZDIRMA ----------------
        bugunun_tarihi = QDateTime.currentDateTime().toString("dd.MM.yyyy HH:mm")

        # Tablo Sırası (8 Sütun): ["#KOD", "AÇIKLAMA", "GRUP", 'AÇIKLAMA2', "RESİM NO", 'AÇIKLAMA SIRA NO', 'AÇIKLAMA SIRA KOD', "KAYIT TARİHİ"]
...
class GovdeGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        ...
        form_layout.addWidget(QLabel("Gövde Boyu:"), 1, 0)
        form_layout.addWidget(self.govde_boyu_kutusu, 1, 1)
        form_layout.addWidget(QLabel("İşlenmiş Durumu:"), 1, 2)
        form_layout.addWidget(self.islenmis_islenmemis, 1, 3)

        form_layout.addWidget(QLabel("Ayak:"), 2, 0)
        form_layout.addWidget(self.ayak_kutusu, 2, 1)
    ...
    def arama_yap(self):
        try:
            ...
            if ek_aciklama:
                # Gelişmiş aramada da "GOVDE" kelimesini temizliyoruz
                ea_temiz = ek_aciklama.replace("GOVDE", "").replace(" ", "")
                if ea_temiz:  # Eğer geriye aranacak bir kelime kaldıysa filtrele

...
class StatorRotorPulArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        baslik = QLabel("STATOR ROTOR PUL")
        ...
        self.kutup_kutusu.addItems(["2", "4", "6", "8", "4/6"])
        ...
        grup_ozel = QGroupBox("Stator\Rotor Pul Detayları")
        duzen_ozel = QGridLayout()

        self.revizyon_kutusu = QComboBox()
        self.revizyon_kutusu.addItems(["01", "02", "03", "04", "05"])

        self.rotor_stator_kutusu = QComboBox()
        self.rotor_stator_kutusu.addItems(["ROTOR", "STATOR"])
        ...
        duzen_ozel.addWidget(QLabel("Rotor/Stator:"), 1, 0)
        duzen_ozel.addWidget(self.rotor_stator_kutusu, 1, 1)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (STATOR ROTOR PUL)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "STATOR-ROTOR PUL"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "STATOR-ROTOR PUL":
                    hedef_sayfa = sayfa
                    break
            ...
            for index, row in df.iterrows():
                ...
                if "-PL-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Format: YMI355-PL-0104-01 -> 2. İndeks: "0104"
                    if len(parcalar) >= 3:
                        sira_kodu_kismi = parcalar[2]
                        if len(sira_kodu_kismi) >= 4 and sira_kodu_kismi.isdigit():
                            gercek_no = sira_kodu_kismi[2:]  # "0104" -> "04"
                            kullanilan_nolar.add(int(gercek_no))
            ...
            try:
                ...
                if "-PL-" in tablo_id:
                    ...
                    if len(parcalar) >= 3:
                        sira_kodu_kismi = parcalar[2]
                        if len(sira_kodu_kismi) >= 4 and sira_kodu_kismi.isdigit():
                            gercek_no = sira_kodu_kismi[2:]
                            kullanilan_nolar.add(int(gercek_no))
            except:
                pass

        # Aradaki kullanılmayan numaraları doldurur
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "STATOR-ROTOR PUL"
        try:
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = StatorRotorPulGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        # 2. KOPYA KONTROLÜ (YIKICI DNA BAZLI - ARAMA MOTORUNUN ZEKASIYLA)
        try:
            df = pd.read_excel(ayar_excel_yolunu_getir(), sheet_name=gercek_sayfa_adi, header=0, dtype=str).fillna("")
            df.columns = [str(c).strip().upper() for c in df.columns]

            # İŞTE ÇÖZÜM: Excel'deki tüm açıklamaları Arama Motorundaki gibi YIKICI DNA'ya çeviriyoruz (Tire ve Slaşlar silinir)
            excel_aciklama_dnalari = []
            for index, row in df.iterrows():
                # İndeks 1 -> Doğrudan AÇIKLAMA sütunudur
                if len(row) > 1:
                    deger = str(row.iloc[1]).strip()
                    dna = turkce_karakter_temizleme(deger).upper().replace(" ", "").replace("-", "").replace("/", "")
                    if dna and dna != "NAN":
            ...
            for satir in range(self.tablo.rowCount()):
                item = self.tablo.item(satir, 1)  # Tabloda 1. İndeks AÇIKLAMA
                if item:
                    # Tablodaki açıklamayı da aynı formatta saf DNA'ya çevir
                    tablo_dna = turkce_karakter_temizleme(item.text().strip()).upper().replace(" ", "").replace("-",
                                                                                                                "").replace(
                        "/", "")

                    # Eğer DNA'lar birebir uyuşuyorsa KESİN kopyadır!
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA STATOR-ROTOR PUL UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki pulun AÇIKLAMASI Excel'de zaten mevcut!\n\n"
            ...
            QMessageBox.warning(self, "Kopya Kontrol Hatası", f"Kopya kontrolü sırasında bir hata oluştu:\n{e}")
            return
        ...
        # 4. GÖLGE KOPYA (SHADOW COPY) VE YAZMA SİSTEMİ
        try:
            temp_klasor = tempfile.gettempdir()
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_pul.xlsx")
            ...
            # 7 Sütunlu tablo, Tarih G8'de başlar.
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_aciklama2):
        # Arama yaparken her ihtimale karşı Türkçe karakterleri düzeltip boşlukları siliyoruz
        ...
        try:
            ...
            for index, row in df.iterrows():
                if len(row) > 4:
                    ...
                    if "-PL-" not in kod_hucre:
                        continue

                    # BÜYÜK ÇÖZÜM: İsimle değil doğrudan indeksle çağırıyoruz!
                    # 3. İndeks = AÇIKLAMA 2 | 4. İndeks = AÇIKLAMA S.NO
    ...
    def uret_calistir(self):
        ...
        rotor_stator = self.rotor_stator_kutusu.currentText().strip()

        ham_ek_aciklama = turkce_karakter_temizleme(self.ek_aciklama_input.text()).strip().upper()
        ek_aciklama = ham_ek_aciklama.replace("PULU", "").strip()
        ...
        if not rotor_stator: bos_alanlar.append("Rotor/Stator")
        ...
        # STATOR İSE BİTİŞİK (355-6), ROTOR İSE BOŞLUKLU (200 -2)
        if rotor_stator == "STATOR":
            baslangic = f"{grup}-{kutup}" if kutup else grup
        else:
            baslangic = f"{grup} -{kutup}" if kutup else grup

        taslak_parcalari.append(baslangic)

        if ek_aciklama:
            taslak_parcalari.append(ek_aciklama)

        taslak_parcalari.append(f"{rotor_stator} PULU")
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Pul ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        # 1. Grubu baştan sil (Örn: "355-6" -> "-6", "200 -2" -> " -2")
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="STATOR-ROTOR PUL", aranan_aciklama2=aciklama2_oto)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="STATOR-ROTOR PUL", id_kodu="PL")
        ...
        aciklama_kod_str = sira_no_str.zfill(2)

        # ---------------- 4. NİHAİ ID OLUŞTURMA ----------------
        id_ortasi = "02" if rotor_stator == "ROTOR" else "01"
        id_revizyon = "00" if versiyon == "ESKİ_GÖVDE" else revizyon

        uretilen_id = f"YMI{grup}-PL-{id_ortasi}{aciklama_kod_str}-{id_revizyon}"
        ...
        yeni_kayit = [
            uretilen_id, taslak_aciklama, rotor_stator, aciklama2_oto, sira_no_str, aciklama_kod_str, bugunun_tarihi
        ]
...
class StatorRotorPulGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("STATOR-ROTOR PUL Gelişmiş Arama")
        ...
        gercek_sayfa = "STATOR-ROTOR PUL"
        try:
            dosya = pd.ExcelFile(self.excel_yolu)
            for s in dosya.sheet_names:
                if s.strip().upper() == "STATOR-ROTOR PUL":
                    gercek_sayfa = s
                    break
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("STATOR-ROTOR PUL Parametreleriyle Filtrele")
        form_layout = QGridLayout()

        # Kutular (Stator-Rotor Pul için Gerekli Olanlar)
        self.versiyon_kutusu = QComboBox()
        self.versiyon_kutusu.addItems(["", "ESKİ_GÖVDE", "PREMIUM"])

        self.grup_kutusu = QComboBox()
        self.grup_kutusu.addItems(
            ["", "063", "071", "080", "090", "100", "112", "132", "160", "180", "200", "225", "250", "280", "315", "355", "400", "450", "1X25 MW"])
        ...
        form_layout.addWidget(QLabel("Rotor/Stator:"), 1, 2)
        form_layout.addWidget(self.rotor_stator_kutusu, 1, 3)
    ...
    def arama_yap(self):
        try:
            ...
            rotor_stator = self.rotor_stator_kutusu.currentText().strip().upper()
            ...
            if kutup:
                df_filtrelenmis = df_filtrelenmis[
                    tum_metin.str.contains(f"-{kutup}", regex=False) | tum_metin.str.contains(f" {kutup} ", regex=False)]

            if rotor_stator:
                df_filtrelenmis = df_filtrelenmis[tum_metin.str.contains(rotor_stator, regex=False)]

...
class FanMuhafazaArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        baslik = QLabel("FAN MUHAFAZA ARAYÜZÜ")
        ...
        self.kutup_kutusu.addItems(["2", "4/6"])

        self.govde_boyu_kutusu = QComboBox()
        self.govde_boyu_kutusu.addItems(["S-M", "M-L"])
        ...
        duzen_temel.addWidget(QLabel("Grup (M):"), 1, 2)
        ...
        grup_ozel = QGroupBox("Fan Muhafaza Detayları")
        duzen_ozel = QGridLayout()

        self.revizyon_kutusu = QComboBox()
        self.revizyon_kutusu.addItems(["01", "02", "03", "04", "05"])

        self.boyali_kutusu = QComboBox()
        self.boyali_kutusu.addItems(["BOYALI", "BOYASIZ"])

        self.boy_input = QLineEdit()
        self.ek_aciklama_input = QLineEdit()

        duzen_ozel.addWidget(QLabel("Boya:"), 0, 0)
        duzen_ozel.addWidget(self.boyali_kutusu, 0, 1)

        duzen_ozel.addWidget(QLabel("Boy:"), 0, 2)
        duzen_ozel.addWidget(self.boy_input, 0, 3)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (FAN MUHAFAZA)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "FAN MUHAFAZA"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "FAN MUHAFAZA":
                    hedef_sayfa = sayfa
                    break
            ...
            for index, row in df.iterrows():
                ...
                if "-FM-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Format: YMI080-FM-0009-00 -> 2. İndeks: "0009"
                    if len(parcalar) >= 3:
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = FanMuhafazaGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA FAN MUHAFAZA UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki muhafaza(lar)ın AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_fm.xlsx")
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_aciklama2):
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama2 == hedef_dna:
                # 7 Sütunlu FM tablosunda SIRA NO 4. İndekstedir!
    ...
    def uret_calistir(self):
        ...
        boyali = self.boyali_kutusu.currentText().strip()
        ...
        if not boyali: bos_alanlar.append("Boya Durumu")
        if not boy: bos_alanlar.append("Boy (MM)")
        ...
        taslak_aciklama = grup

        if kutup:
            taslak_aciklama += f"-{kutup}"

        if govde_boyu:
            taslak_aciklama += f" {govde_boyu}"

        taslak_aciklama += " FAN MUHAFAZA "

        if ek_aciklama:
            taslak_aciklama += f"- {ek_aciklama} - "

        # Boyalı/Boyasız, Boy ve MM ekle
        taslak_aciklama += f"{boyali} {boy} MM"
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Fan Muhafaza ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return

        # --- AÇIKLAMA 2'Yİ OTOMATİK ÜRETMESİ ---
        aciklama2_oto = taslak_aciklama

        # 1. Grubu baştan sil ve geriye kalan saçma tireyi temizle
        if aciklama2_oto.startswith(grup):
            aciklama2_oto = aciklama2_oto[len(grup):].strip()
            # Başta tire (-) veya boşluk kaldıysa tamamen yok olana kadar temizle:
            while aciklama2_oto.startswith("-") or aciklama2_oto.startswith(" "):
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="FAN MUHAFAZA", aranan_aciklama2=aciklama2_oto)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="FAN MUHAFAZA", id_kodu="FM")
        ...
        uretilen_id = f"YMI{grup}-FM-{aciklama_kod_str}-{id_revizyon}"
...
class FanMuhafazaGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("FAN MUHAFAZA Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("Fan Muhafaza Parametreleriyle Filtrele")
        form_layout = QGridLayout()

        # Kutular (Ana forma birebir uyumlu)
        self.versiyon_kutusu = QComboBox()
        self.versiyon_kutusu.addItems(["", "ESKİ_GÖVDE", "PREMIUM"])
        ...
        self.boy_input = QLineEdit()
        self.boy_input.setPlaceholderText("Boy (MM)")
        ...
        form_layout.addWidget(QLabel("Boya:"), 2, 0)
        form_layout.addWidget(self.boyali_kutusu, 2, 1)
        form_layout.addWidget(QLabel("Boy (MM):"), 2, 2)
        form_layout.addWidget(self.boy_input, 2, 3)
    ...
    def arama_yap(self):
        try:
            ...
            boyali = self.boyali_kutusu.currentText().strip().upper()
            boy = self.boy_input.text().strip().upper()
            ...
            if kutup:
                df_filtrelenmis = df_filtrelenmis[tum_metin.str.contains(f"-{kutup}", regex=False)]

            if govde_boyu:
                gb_temiz = govde_boyu.replace("-", "")
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(gb_temiz, regex=False)]

            if boyali:
                if boyali == "BOYALI":
                    df_filtrelenmis = df_filtrelenmis[
                        metin_sifir_bosluk.str.contains("BOYALI", regex=False) &
                        ~metin_sifir_bosluk.str.contains("BOYASIZ", regex=False)
                    ]
                else:
                    df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("BOYASIZ", regex=False)]

            if boy:
                # "250 MM" gibi değerleri aramak için
                boy_tam = f"{boy}MM"
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(boy_tam, regex=False)]

...
class GovdeAyakArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        self.govde_boyu_kutusu.addItems(["", "S", "M", "L", "S-M", "S-L", "M-L", "S-M-L","340"])


        duzen_temel.addWidget(QLabel("Grup (M):"), 0, 0 )
        duzen_temel.addWidget(self.grup_kutusu, 0, 1)

        duzen_temel.addWidget(QLabel("Govde Boyu:"), 0, 2)
        duzen_temel.addWidget(self.govde_boyu_kutusu, 0, 3)


        grup_temel.setLayout(duzen_temel)
        form_ana_duzen.addWidget(grup_temel)

        # --- GRUP 2: Gövde Ayak Özel Girdiler ---
        grup_ozel = QGroupBox("Gövde AYAK Detayları")
        ...
        self.islenmis_islenmemis = QComboBox()
        self.islenmis_islenmemis.addItems(["ISLENMIS", "ISLENMEMIS","YARI ISLENMIS"])  # Harf hatası düzeltildi

        # SAĞ/SOL KURALI: Eğer iki ayak da varsa boş ("") bırakılacak!
        self.sag_sol_kutusu = QComboBox()
        self.sag_sol_kutusu.addItems(["", "SAG", "SOL"])
        ...
        duzen_ozel.addWidget(self.islenmis_islenmemis, 1, 1)

        duzen_ozel.addWidget(QLabel("SAG/SOL:"), 1, 2)
        duzen_ozel.addWidget(self.sag_sol_kutusu, 1, 3)

        duzen_ozel.addWidget(QLabel("Gövde Malzemesi:"), 2, 0)
        duzen_ozel.addWidget(self.govde_malzemesi_kutusu, 2, 1)

        duzen_ozel.addWidget(QLabel("Resim No:"), 2, 2)
        duzen_ozel.addWidget(self.resim_no_kutusu, 2, 3)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (GOVDE AYAK)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "GOVDE AYAK"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "GOVDE AYAK":
                    hedef_sayfa = sayfa
                    break
            ...
            for idx, row in df_veri.iterrows():
                ...
                if aciklama_str:  # Açıklama doluysa numarayı al
                    # FORMAT NE OLURSA OLSUN (GA, PB, VS) ORTADAKİ 4 HANELİ KODU YAKALAR
                    eslesme = re.search(r'-[A-Z]+-(\d{4})-', val_str)
                    if eslesme:
                        kullanilan_nolar.add(int(eslesme.group(1)))
        ...
        for row_idx in range(self.tablo.rowCount()):
            try:
                ...
                eslesme = re.search(r'-[A-Z]+-(\d{4})-', tablo_id)
                if eslesme:
                    kullanilan_nolar.add(int(eslesme.group(1)))
            except:
                pass

        # 3. İLK BOŞLUĞU BUL (Atlanan numaraları kusursuz doldurur)
    ...
    def tabloda_arama_yap(self):
        ...
        # --- DÜZELTME: GÖVDE KELİMESİNİ GİZLİCE SİLİYORUZ ---
        # Excel'deki kayıtlarda "GOVDE" geçmediği için, kullanıcı "gövde ayak" yazarsa
        # sadece "AYAK" kısmını aramasını sağlıyoruz.
        aranan_aciklama = aranan_aciklama.replace("GOVDE AYAK", "AYAK").replace("GOVDE", "").strip()
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = GovdeAyakGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA GÖVDE AYAK UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki ayak(lar)ın AYNISI Excel'de zaten mevcut!\n\n"
                ...
                if cevap_kopya == QMessageBox.StandardButton.Cancel:
                    ...
                    if self.tablo.rowCount() == 0:
                        QMessageBox.information(self, "Bilgi", "Kopyalar silinince tabloda kaydedilecek yeni veri kalmadı.")
                        return
        except Exception as e:
            pass

        cevap = QMessageBox.question(self, "ONAY", f"{gercek_sayfa_adi} sayfasına aktarılsın mı?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_govde_ayak.xlsx")
            ...
            return  # Hata varsa işlemi durdur ve alttaki kopyalama işlemine geçme

        finally:
            # NE OLURSA OLSUN EXCEL'İ ARKA PLANDA KAPAT (ÇÖKMEYİ ENGELLER)
            if app is not None:
                try:
                    app.quit()
                except:
        ...
        # Gecici Excel basariyla kapatildiysa ana dosyaya tasi
        try:
            ...
            QMessageBox.critical(self, "DOSYA KİLİTLİ!",
                                 "Excel dosyası şu an AÇIK olduğu için yazılamadı. Lütfen kapatıp tekrar deneyin.")
        except Exception as e:
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_grup, taslak_metin):
        def kelime_kumesi_yap(metin):
            temiz = turkce_karakter_temizleme(str(metin)).upper()  # DÜZELTME: TR karakter normalize edilmeden karşılaştırılıyordu

            # --- YENİ EKLENEN: REVİZYON NO VE RESİM NO'YU DNA'DAN SİLİP ATMA KURALI ---
            temiz = re.sub(r'R\s*:\s*\d+', '', temiz)
            temiz = re.sub(r'\b\d{4,5}\b', '', temiz)  # 3944, 0000 gibi resim numaralarını yutar!
        ...
        aranan_grup_kumesi = kelime_kumesi_yap(aranan_grup)

        # --- 1. ARAYÜZ TABLOSU ---
        for row_idx in range(self.tablo.rowCount()):
            tablo_grup = str(self.tablo.item(row_idx, 2).text()).strip()

            if kelime_kumesi_yap(tablo_grup) == aranan_grup_kumesi:
                tab_aciklama = str(self.tablo.item(row_idx, 1).text())
                tab_aciklama2 = str(self.tablo.item(row_idx, 3).text())
                tablo_dna_kumesi = kelime_kumesi_yap(f"{tab_aciklama} {tab_aciklama2}")

                if programin_dna_kumesi == tablo_dna_kumesi:
                    # Tabloda 5. indeks SIRA NO
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                if kelime_kumesi_yap(excel_grup) == aranan_grup_kumesi:
                    excel_aciklama = str(row.get('AÇIKLAMA', ''))
                    excel_aciklama2 = str(row.get('AÇIKLAMA2', row.get('EK AÇIKLAMA', '')))
                    excel_dna_kumesi = kelime_kumesi_yap(f"{excel_aciklama} {excel_aciklama2}")

                    if programin_dna_kumesi == excel_dna_kumesi:
    ...
    def uret_calistir(self):
        ...
        islenmis = self.islenmis_islenmemis.currentText().strip()
        sag_sol = self.sag_sol_kutusu.currentText().strip()
        malzeme = self.govde_malzemesi_kutusu.currentText().strip()
        ...
        ek_aciklama = ham_ek_aciklama.replace("AYAK", "").replace("GOVDE", "").strip()
        ...
        if not malzeme: bos_alanlar.append("Gövde Malzemesi")
        if not revizyon: bos_alanlar.append("Revizyon No")
        if not resim_no: bos_alanlar.append("Resim No")
        # DİKKAT: sag_sol kutusunu bilerek çıkardım çünkü TAKIM olduğunda boş kalması gerekiyor!
        ...
        if govde_boyu:
            taslak_parcalari.append(govde_boyu)
        ...
        if sag_sol:
            taslak_parcalari.append(sag_sol)

        taslak_parcalari.append("AYAK")
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Gövde Ayak ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        # 3. Resim numarasını sondan sil (Açıklama 2'ye geçmesini istemeyiz)
        if resim_no and resim_no != "0000":
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="GOVDE AYAK", aranan_grup=grup, taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, taslak_aciklama, sayfa_adi="GOVDE AYAK", id_kodu="GA")
        ...
        uretilen_id = f"YMI{grup}-GA-{aciklama_kod_str}-{revizyon}-{id_resim}"

        # ---------------- 5. TABLOYA YAZDIRMA ----------------
        bugunun_tarihi = QDateTime.currentDateTime().toString("dd.MM.yyyy HH:mm")

        # Tablo Sırası (8 Sütun)
        yeni_kayit = [
...
class GovdeAyakGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Gövde Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("Gövde Parametreleriyle Filtrele")
        ...
        self.islenmis_islenmemis = QComboBox()
        self.islenmis_islenmemis.addItems(["", "ISLENMIS", "ISLENMEMIS","YARI ISLENMIS"])

        self.ayak_kutusu = QComboBox()
        self.ayak_kutusu.addItems(["", "SAG", "SOL"])
        ...
        # Yerleştirme
        form_layout.addWidget(QLabel("Grup (M):"), 0, 0)
        form_layout.addWidget(self.grup_kutusu, 0, 1)

        form_layout.addWidget(QLabel("Gövde Boyu:"), 0, 2)
        form_layout.addWidget(self.govde_boyu_kutusu, 0, 3)

        form_layout.addWidget(QLabel("İşlenmiş Durumu:"), 1, 0)
        form_layout.addWidget(self.islenmis_islenmemis, 1, 1)

        form_layout.addWidget(QLabel("Ayak:"), 1, 2)
        form_layout.addWidget(self.ayak_kutusu, 1, 3)

        form_layout.addWidget(QLabel("Malzeme:"), 2, 0)
        form_layout.addWidget(self.govde_malzemesi_kutusu, 2, 1)
        form_layout.addWidget(QLabel("Resim No:"), 2, 2)
        form_layout.addWidget(self.resim_no_input, 2, 3)
        ...
        try:
            ...
            if islenmis:
                ...
                if islenmis_temiz == "ISLENMIS":
                    # DİKKAT: Sadece ISLENMIS olanı bulmalı, ISLENMEMIS veya YARIISLENMIS olanları listeden atmalı!
                    df_filtrelenmis = df_filtrelenmis[
                        ...
                        ~metin_sifir_bosluk.str.contains("YARIISLENMIS", regex=False)
                        ]
                else:
                    df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(islenmis_temiz, regex=False)]

            if ayak:
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(ayak, regex=False)]
            ...
            if ek_aciklama:
                # Gelişmiş aramada da "GOVDE" kelimesini gizlice temizliyoruz
                ea_temiz = ek_aciklama.replace("GOVDE", "").replace(" ", "")
                # Eğer geriye aranacak bir kelime kaldıysa filtrele!
                if ea_temiz:

...
class KapakArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        grup_ozel = QGroupBox("Kapak Detayları")
        ...
        self.kapak_malzemesi_kutusu = QComboBox()
        self.kapak_malzemesi_kutusu.addItems(["ALUMINYUM", "PIK", "CELIK"])

        self.kapak_tarafi_kutusu = QComboBox()
        self.kapak_tarafi_kutusu.addItems(["KT", "FT", "FT-KT"])
        ...
        duzen_ozel.addWidget(QLabel("Kapak Malzemesi:"), 1, 2)
        duzen_ozel.addWidget(self.kapak_malzemesi_kutusu, 1, 3)

        duzen_ozel.addWidget(QLabel("Resim No:"), 2, 0)
        duzen_ozel.addWidget(self.resim_no_kutusu, 2, 1)

        duzen_ozel.addWidget(QLabel("Kapak Tarafı:"), 2, 2)
        duzen_ozel.addWidget(self.kapak_tarafi_kutusu, 2, 3)
        ...
        # 1. Satır
        duzen_onaylar.addWidget(self.cb_derin, 0, 0)
        duzen_onaylar.addWidget(self.cb_burclu, 0, 1)
        duzen_onaylar.addWidget(self.cb_guduk, 0, 2)
        duzen_onaylar.addWidget(self.cb_segmanli, 0, 3)

        # 2. Satır
        duzen_onaylar.addWidget(self.cb_yag_keceli, 1, 0)
        duzen_onaylar.addWidget(self.cb_drenaj_tapali, 1, 1)
        duzen_onaylar.addWidget(self.cb_yaglamali, 1, 2)
        ...
        # ---------------- TABLO ----------------
        self.tablo = QTableWidget(0, 9)  # 7 YERİNE 9 OLDU!
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (KAPAK)")
    ...
    def formu_temizle(self):
        ...
        for nesne in self.findChildren(QLineEdit):
            nesne.clear()  # Arama kutuları da siliniyor!
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                if "-KP-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Örn: YMI063-KP-0002-00-0000 -> 2. İndeks: "0002"
                    if len(parcalar) >= 3:
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = KapakGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA KAPAK UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki kapak(lar)ın AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_kapak.xlsx")
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_aciklama2):
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama2 == hedef_dna:
                # 9 Sütunlu Kapak tablosunda RESİM NO 4. İndekste olduğu için, SIRA NO 5. İndekstedir!
    ...
    def uret_calistir(self):
        ...
        malzeme = self.kapak_malzemesi_kutusu.currentText().strip()
        kapak_tarafi = self.kapak_tarafi_kutusu.currentText().strip()
        ...
        if not kapak_tarafi: bos_alanlar.append("Kapak Tarafı")
        ...
        # SABİT MÜHENDİSLİK SIRASI: DERIN BURCLU GUDUK SEGMANLI YAG KECELI DRENAJ TAPALI YAGLAMALI
        ek_ozellikler = []
        if self.cb_derin.isChecked(): ek_ozellikler.append("DERIN")
        ...
        # Elle yazılan açıklama ile tiklenenleri çakışmadan birleştir
        ...
        if kapak_tarafi:
            taslak_parcalari.append(kapak_tarafi)
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Kapak ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="KAPAK", aranan_aciklama2=aciklama2_oto)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="KAPAK", id_kodu="KP")
...
class KapakGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("KAPAK Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI ---
        grup_form = QGroupBox("KAPAK Parametreleriyle Filtrele")
        ...
        self.grup_kutusu.addItems(
            ["", "063", "071", "080", "090", "100", "112", "132", "160", "180", "200", "225", "250", "280", "315",
             "355", "400", "450", "500", "630", "063-090", "071-090", "100-112", "100-132", "160-180", "180-225",
             "355-400"])
        ...
        form_layout.addWidget(QLabel("Kapak Tarafı:"), 1, 0)
        form_layout.addWidget(self.kapak_tarafi_kutusu, 1, 1)
        form_layout.addWidget(QLabel("İşlenmiş Durumu:"), 1, 2)
        form_layout.addWidget(self.islenmis_islenmemis_kutusu, 1, 3)

        form_layout.addWidget(QLabel("Kapak Malzemesi:"), 2, 0)
        form_layout.addWidget(self.kapak_malzemesi_kutusu, 2, 1)
    ...
    def arama_yap(self):
        try:
            ...
            if kapak_tarafi:
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains(kapak_tarafi, regex=False)]

...
class GovdeAyakTakimArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        baslik = QLabel("GOVDE AYAK TAKIM")
        ...
        # --- GRUP 2: GOVDE AYAK TAKIM Detayları ---
        grup_ozel = QGroupBox("GOVDE AYAK TAKIM Detayları")
        ...
        duzen_ozel.addWidget(QLabel("Gövde Malzemesi:"), 1, 0)
        duzen_ozel.addWidget(self.govde_malzemesi_kutusu, 1, 1)
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (GOVDE AYAK TAKIM)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "GOVDE AYAK TAKIM"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "GOVDE AYAK TAKIM":
                    hedef_sayfa = sayfa
                    break
            ...
            # HAYALET SATIR KORUMASI
            for idx, row in df.iterrows():
                val_str = str(row.iloc[0]).strip().upper()
                aciklama_str = str(row.iloc[1]).strip()

                if "GAT-" in val_str and aciklama_str:
                    parcalar = val_str.split("-")
                    # Format: YMM160-GAT-0001-01 -> 2. İndeks her zaman SIRA KODUDUR
                    if len(parcalar) >= 3 and parcalar[2].isdigit():
                        kullanilan_nolar.add(int(parcalar[2]))
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = GovdeAyakTakimGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if temiz_sayfa == "GOVDE AYAK TAKIM":
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA GÖVDE AYAK TAKIM UYARISI!",
                    f"Tablodaki {satir_numaralari}. satırdaki takım(lar)ın AYNISI Excel'de zaten mevcut!\n\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_gat.xlsx")
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if kelime_kumesi_yap(tablo_grup) == aranan_grup_kumesi:
                ...
                if programin_dna_kumesi == tablo_dna_kumesi:
                    # GÖVDE AYAK TAKIM 7 SÜTUNDUR (Resim no yok). SIRA NO 4. İndekste!
    ...
    def uret_calistir(self):
        ...
        ek_aciklama = ham_ek_aciklama.replace("GOVDE", "").replace("AYAK", "").replace("TAKIM", "").strip()
        ...
        if govde_boyu: taslak_parcalari.append(govde_boyu)
        if ek_aciklama: taslak_parcalari.append(ek_aciklama)
        if malzeme: taslak_parcalari.append(malzeme)

        taslak_parcalari.append(f"GOVDE AYAK - TAKIM - R:{revizyon}")
        ...
        for r in range(self.tablo.rowCount()):
            ...
            if tablo_aciklama == taslak_aciklama:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu AÇIKLAMAYA sahip Gövde Ayak Takım ZATEN LİSTEDE var!\n\nAynı özellikleri ard arda ekleyemezsiniz.")
                return
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="GOVDE AYAK TAKIM", aranan_grup=grup,
                                              taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, taslak_aciklama, sayfa_adi="GOVDE AYAK TAKIM", id_kodu="GAT")
        ...
        uretilen_id = f"YMM{grup}-GAT-{aciklama_kod_str}-{revizyon}"
...
class GovdeAyakTakimGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Gövde Ayak Takım Gelişmiş Arama")
        ...
        # --- 2. FORM ALANI (SADECE TAKIM İÇİN GEREKLİ OLANLAR) ---
        grup_form = QGroupBox("Gövde Ayak Takım Parametreleriyle Filtrele")
        ...
        form_layout.addWidget(QLabel("Malzeme:"), 1, 0)
        form_layout.addWidget(self.govde_malzemesi_kutusu, 1, 1)

        form_layout.addWidget(QLabel("Revizyon No:"), 1, 2)
        form_layout.addWidget(self.revizyon_kutusu, 1, 3)

        form_layout.addWidget(QLabel("Ek Açıklama:"), 2, 0)
        form_layout.addWidget(self.ek_aciklama_input, 2, 1)

        form_layout.addWidget(QLabel("Sıra No (Kod):"), 2, 2)
        form_layout.addWidget(self.sira_no_kutusu, 2, 3)
    ...
    def arama_yap(self):
        try:
            ...
            # Form Girdilerini Çek (SADECE TAKIM İÇİN GEREKLİ OLANLAR)

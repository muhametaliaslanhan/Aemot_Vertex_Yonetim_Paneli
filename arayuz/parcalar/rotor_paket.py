...
class RotorPaketArayuzu(QWidget):
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "ROTOR PAKET":
                    hedef_sayfa = sayfa
                    break
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "ROTOR PAKET"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                s_up = sayfa.upper().strip()
                if "ROTOR" in s_up and "PAKET" in s_up:
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = RotorPaketGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        # 1. KOPYA KONTROLÜ
        try:
            # Doğrudan gercek_sayfa_adi ile A1'den temiz okuma
            ...
            if cakisan_satirlar:
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA ROTOR PAKET UYARISI!",
        ...
        # 2. KAYIT ONAYI
        excel_aktarim_cevap = QMessageBox.question(self, "KAYIT ONAYI", f"Tablodaki verileri Excel'e ({gercek_sayfa_adi}) GÖNDERMEK istiyor musunuz?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_rotor_paket.xlsx")
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_grup, taslak_metin, resim_no_cek=False):
        ...
        aranan_grup_dizisi = kelime_dizisi_yap(aranan_grup)

        en_yakin_sira = None
        en_yakin_fark = 999
        en_yakin_fark_kelimeler = None

        # --- 1. ARAYÜZ TABLOSU ---
        for row_idx in range(self.tablo.rowCount()):
            tablo_grup = str(self.tablo.item(row_idx, 2).text()).strip().upper()

            if aranan_grup_dizisi.issubset(kelime_dizisi_yap(tablo_grup)):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                if aranan_grup_dizisi.issubset(kelime_dizisi_yap(excel_grup)):
                    ...
                    fark_sayisi = len(programin_dizisi.symmetric_difference(excel_dizisi))
                    if fark_sayisi < en_yakin_fark and fark_sayisi < 4:
                        en_yakin_fark = fark_sayisi
                        en_yakin_fark_kelimeler = programin_dizisi.symmetric_difference(excel_dizisi)
                        ...
                        if sira_kolonu not in df.columns:
                            for col in df.columns:
                                if "SIRA" in col or "NO" in col and "RESİM" not in col:
                                    sira_kolonu = col
                                    break
                        en_yakin_sira = str(row.get(sira_kolonu, 'Bulunamadı'))
        ...
        if en_yakin_fark < 4:
            print("\n" + "!" * 50)
            print(f"🕵️ DNA X-RAY TESPİTİ! (Neden Yeni Kod Verdim?)")
            print(f"Senin açıklamana en çok benzeyen Excel Satırı No: {en_yakin_sira}")
            print(f"Eşleşmeyi bozan farklı kelimeler: {en_yakin_fark_kelimeler}")
            print("!" * 50 + "\n")
    ...
    def uret_calistir(self):
        ...
        secilen_sira = sira_no_secim_yap(self, bulunan_eski_sira_no, taslak_aciklama, sayfa_adi="ROTOR PAKET", id_kodu="RP")
        ...
        for r in range(self.tablo.rowCount()):
            if self.tablo.item(r, 0) and self.tablo.item(r, 0).text().strip() == uretilen_id:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu Rotor Paket ZATEN LİSTEDE var!\nID: {uretilen_id}\n\nAynı paketi ard arda ekleyemezsiniz.")
                return
...
class RotorPaketGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Rotor Paket Gelişmiş Arama")
        ...
        gercek_sayfa = "ROTOR PAKET"
        try:
            dosya = pd.ExcelFile(self.excel_yolu)
            for s in dosya.sheet_names:
                # DİKKAT: 'or' yerine 'and' yapıyoruz ki Rotorlu Mil'e atlamasın!
                if "ROTOR" in s.upper() and "PAKET" in s.upper():
                    gercek_sayfa = s
                    break
        ...
        grup_form = QGroupBox("Rotor Paket Parametreleriyle Filtrele")
        ...
        form_layout.addWidget(QLabel("Paket Boyu (PB) [mm]:"), 2, 0)
        form_layout.addWidget(self.paket_boyu_input, 2, 1)
    ...
    def arama_yap(self):
        try:
            ...
            # Form Girdilerini Çek (Rotor Paket Parametreleri)
            ...
            if paket_boyu:
                df_filtrelenmis = df_filtrelenmis[
                    tum_metin.str.contains(f"PB:{paket_boyu}", regex=False) | tum_metin.str.contains(f"PB: {paket_boyu}", regex=False)]

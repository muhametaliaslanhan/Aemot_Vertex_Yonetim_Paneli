...
class BobinliStatorArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        grup_temel = QGroupBox("Temel Bobinli Stator Özellikleri")
        ...
        duzen_opsiyon.addWidget(self.resim_no_kutusu,0, 3)
        ...
        duzen_opsiyon.addWidget(QLabel("Revizyon No:"), 0, 0 )
        duzen_opsiyon.addWidget(self.revizyon_kutusu, 0, 1 )

        duzen_opsiyon.addWidget(QLabel("Özel Frekans:"), 2, 2)
        duzen_opsiyon.addWidget(self.ozel_frekans_kutusu, 2, 3)

        duzen_opsiyon.addWidget(QLabel("Özel Gerilim:"), 2, 0)
        duzen_opsiyon.addWidget(self.ozel_gerilim_kutusu, 2, 1)
        ...
        # ---------------- TABLO ----------------
        self.tablo = QTableWidget(0, 7)  # DİKKAT: 8 DEĞİL 7 OLDU!
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (BOBINLI STATOR)")
    ...
    def formu_temizle(self):
        ...
        for nesne in self.findChildren(QLineEdit):
            nesne.clear()  # Arama kutularını da tertemiz yapar!
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "BOBINLI STATOR"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "BOBINLI STATOR":
                    hedef_sayfa = sayfa
                    break
    ...
    def tabloda_arama_yap(self):
        ...
        gercek_sayfa_adi = "BOBINLI STATOR"
        try:
            excel_dosyasi = pd.ExcelFile(guncel_yol)
            for sayfa in excel_dosyasi.sheet_names:
                temiz_sayfa_adi = sayfa.strip().upper()
                if "BOBINLI" in temiz_sayfa_adi and "STATOR" in temiz_sayfa_adi:
        ...
        try:
            ...
            self.tablo.setSortingEnabled(True)
            QMessageBox.warning(self, "Hata", f"Excel dosyasında 'BOBINLI STATOR' sayfası bulunamadı!")
        except Exception as e:
            self.tablo.setSortingEnabled(True)
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = BobinliStatorGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                ...
                if "BOBINLI" in temiz_sayfa and "STATOR" in temiz_sayfa:
                    gercek_sayfa_adi = sayfa
                    break
        except Exception as e:
        ...
        try:
            ...
            if cakisan_satirlar:
                ...
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA BOBİNLİ STATOR UYARISI!",
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_bobinli_stator.xlsx")
            ...
            # Bobinli Stator tablosu 7 sütundur (A B C D E F G). G Sütunu = Kayıt Tarihi
            ...
            try:
                ...
                QMessageBox.information(self, "Başarılı", "Kayıtlar Excel'in 9. satırından itibaren kusursuzca eklendi!")
                self.tablo.setRowCount(0)

            except PermissionError:
                QMessageBox.critical(self, "DOSYA KİLİTLİ!", "Veriler hazırlandı ancak Excel dosyası şu an AÇIK olduğu için yazılamadı. Lütfen kapatıp tekrar deneyin.")
            except Exception as e:
    ...
    def dna_bazli_sira_no_bul(self, sayfa_adi, aranan_grup, taslak_metin):
        ...
        for row_idx in range(self.tablo.rowCount()):
            ...
            if kelime_dizisi_yap(tablo_grup) == kelime_dizisi_yap(aranan_grup):
                ...
                if programin_dizisi == tablo_dizisi:
                    # Tabloda 4. indeks SIRA NO (Çünkü Resim No yok, sütunlar kaydı)
        ...
        try:
            ...
            for sayfa in excel_dosyasi.sheet_names:
                if "BOBINLI" in sayfa.upper() and "STATOR" in sayfa.upper():
                    hedef_sayfa = sayfa
                    break
    ...
    def uret_calistir(self):
        ...
        # --- YENİ EKLENEN: BOŞ ALAN UYARISI (Verimlilik dahil) ---
        bos_alanlar = []
        ...
        yasakli_kelimeler = ["MARIN", "MARINE", "TERMISTOR", "TERMIK", "ISITICI", "GUDUK", "-V", "PASLANMAZ", "HZ",
        ...
        # -V (Vantilatör) eklentisi paket boyundan hemen sonraya taşındı!
        baslangic = f"{grup} {govde_boyu}{kutup}{paket_boyu}"
        if vantilator: baslangic += "-V"

        orta_1 = f" {verimlilik}" if verimlilik else ""

        # DİKKAT: "if not standart:" kısıtlaması kırıldı! Checkbox varsa kesinlikle açıklamaya eklenecek.
        orta_2_parcalar = []
        ...
        sinif = "H" if h_class else "F"
        sabit_bitis = f" {sinif} CLASS BOBINLI STATOR"
        ...
        # KW tire işaretinden öncesini makaslama mantığı
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="BOBINLI STATOR", aranan_grup=grup,
                                              taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, taslak_aciklama, sayfa_adi="BOBINLI STATOR", id_kodu="BS")
        ...
        uretilen_id = f"YMM{grup}-BS-{aciklama_kod_str}-{id_revizyon}"

        for r in range(self.tablo.rowCount()):
            if self.tablo.item(r, 0) and self.tablo.item(r, 0).text().strip() == uretilen_id:
                QMessageBox.warning(self, "KOPYA ENGELİ", f"Bu Bobinli Stator ZATEN LİSTEDE var!\nID: {uretilen_id}")
                return
...
class BobinliStatorGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Bobinli Stator Gelişmiş Arama") # Başlık Düzeltildi!
        ...
        gercek_sayfa = "BOBINLI STATOR" # Sayfa Adı Düzeltildi!
        try:
        ...
        # --- 2. FORM ALANI (BOBINLI STATOR ANA FORMUNUN ARAMA VERSİYONU) ---
        grup_form = QGroupBox("Bobinli Stator Parametreleriyle Filtrele")
        ...
        self.kutup_kutusu = QComboBox()
        self.kutup_kutusu.addItems(
            ["", "2", "4", "6", "8", "10", "12", "2/4", "4/2", "4/8", "6/4", "6/8", "8/2", "8/4", "8/6", "2/12",
             "4/12",
             "12/4", "4/16"])
        ...
        self.ek_aciklama_input.setPlaceholderText("Ek Açıklama (Örn: PT100, ÖZEL MİL)")
        ...
        form_layout.addWidget(QLabel("Ek Açıklama:"), 3, 0)
        form_layout.addWidget(self.ek_aciklama_input, 3, 1)
        form_layout.addWidget(QLabel("Sıra No (Kod):"), 3, 2)
        form_layout.addWidget(self.sira_no_kutusu, 3, 3)

        # YENİ EKLENEN: ONAY KUTULARI (Checkboxlar)
        self.cb_isitici = QCheckBox("ISITICI")
        self.cb_termistor = QCheckBox("TERMİSTÖR")
        self.cb_vantilator = QCheckBox("VANTİLATÖR")
        self.cb_termik = QCheckBox("TERMİK")
        ...
        checkbox_duzen = QGridLayout()
        checkbox_duzen.addWidget(self.cb_isitici, 0, 0)
        checkbox_duzen.addWidget(self.cb_termistor, 0, 1)
        checkbox_duzen.addWidget(self.cb_vantilator, 0, 2)
        checkbox_duzen.addWidget(self.cb_termik, 1, 0)
        checkbox_duzen.addWidget(self.cb_h_class, 1, 1)
        checkbox_duzen.addWidget(self.cb_tropikalizeli, 1, 2)

        # Checkboxları ana forma dahil et (4. satır)
        form_layout.addLayout(checkbox_duzen, 4, 0, 1, 4)
    ...
    def arama_yap(self):
        try:
            ...
            if paket_boyu:
                df_filtrelenmis = df_filtrelenmis[tum_metin.str.contains(f"PB:{paket_boyu}", regex=False) | tum_metin.str.contains(f"PB: {paket_boyu}", regex=False)]

            if revizyon:
                df_filtrelenmis = df_filtrelenmis[tum_metin.str.contains(f"R:{revizyon}", regex=False) | tum_metin.str.contains(f"-{revizyon}-", regex=False)]
            ...
            # Checkbox Aramaları
            if self.cb_isitici.isChecked():
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("ISITICILI", regex=False)]
            if self.cb_termistor.isChecked():
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("TERMISTORLU", regex=False)]
            if self.cb_vantilator.isChecked():
                df_filtrelenmis = df_filtrelenmis[tum_metin.str.contains("-V", regex=False)]
            if self.cb_termik.isChecked():
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("TERMIKLI", regex=False)]
            if self.cb_h_class.isChecked():
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("HCLASS", regex=False)]
            if self.cb_tropikalizeli.isChecked():
                df_filtrelenmis = df_filtrelenmis[metin_sifir_bosluk.str.contains("TROPIKALIZELI", regex=False)]

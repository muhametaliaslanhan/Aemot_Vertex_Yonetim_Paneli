...
class RotorSaciArayuzu(QWidget):
    def __init__(self, ana_pencere):
        ...
        self.grup_kutusu.addItems(
            ["063", "071", "080", "090", "100", "112", "132", "160", "180", "200", "225", "250", "280", "315", "355",
             "400", "450", "500", "630","NORDEX","1 X 25MW"])
        ...
        grup_ozel = QGroupBox("Rotor Sac Detayları")
        ...
        self.aktar_butonu = QPushButton("TÜMÜNÜ EXCEL'E AKTAR (ROTOR SACI)")
    ...
    def genel_max_sira_no_bul(self):
        ...
        try:
            ...
            hedef_sayfa = "ROTOR SACI"
            for sayfa in excel_dosyasi.sheet_names:
                if sayfa.strip().upper() == "ROTOR SACI":
                    hedef_sayfa = sayfa
                    break
        ...
        try:
            ...
            for index, row in df_veri.iterrows():
                ...
                if id_eslesti and aciklama_eslesti:
                    ...
                    for col_idx in range(min(7, len(row))):
    ...
    def gelismis_arama_ac(self):
        try:
            ...
            dialog = RotorSaciGelismisArama(self, excel_yolu)
            dialog.exec()
        except Exception as e:
    ...
    def excele_aktar(self):
        ...
        try:
            ...
            if cakisan_satirlar:
                cevap_kopya = QMessageBox.question(
                    self, "KOPYA ROTOR SACI UYARISI!",
                    f"Tablodaki {len(cakisan_satirlar)} adet kayıt Excel'de zaten mevcut!\n\n"
                    f"• EVET: Kopyaları tablodan kaldır ve kalanları kaydet.\n"
        ...
        try:
            ...
            gecici_excel = os.path.join(temp_klasor, "gecici_aemot_rotor_saci.xlsx")
    ...
    def uret_calistir(self):
        ...
        ek_aciklama = ham_ek_aciklama.replace("ROTOR SACI", "").strip()
        ...
        taslak_parcalari.append("ROTOR SACI")

        if versiyon != "ESKİ_GÖVDE":
            taslak_parcalari.append(f"- R:{revizyon}")

        taslak_aciklama = " ".join(taslak_parcalari)
        taslak_aciklama = " ".join(taslak_aciklama.split())  # Fazla boşlukları sil
        ...
        dna_sira = self.dna_bazli_sira_no_bul(sayfa_adi="ROTOR SACI", aranan_grup=grup, taslak_metin=tam_metin_havuzu)

        from arayuz.mantik.sira_no_dialog import sira_no_secim_yap
        secilen_sira = sira_no_secim_yap(self, dna_sira, aciklama2_oto, sayfa_adi="ROTOR SACI", id_kodu="RS")
        ...
        uretilen_id = f"YMI{grup}-RS-{aciklama_kod_str}-{id_revizyon}"
        ...
        for r in range(self.tablo.rowCount()):
            if self.tablo.item(r, 0) and self.tablo.item(r, 0).text().strip() == uretilen_id:
                QMessageBox.warning(self, "KOPYA ENGELİ",
                                    f"Bu Rotor Sacı ZATEN LİSTEDE var!\nID: {uretilen_id}\n\nAynı rotoru ard arda ekleyemezsiniz.")
                return
        ...
        yeni_kayit = [
            uretilen_id, taslak_aciklama, grup, aciklama2_oto, sira_no_str, f"{aciklama_kod_str}", bugunun_tarihi
        ]
...
class RotorSaciGelismisArama(QDialog):
    def __init__(self, parent, excel_yolu):
        super().__init__(parent)
        self.setWindowTitle("Rotor Sacı Gelişmiş Arama") # Başlık Düzeltildi
        ...
        gercek_sayfa = "ROTOR SACI"  # Excel Sayfa Adı Düzeltildi
        try:
            dosya = pd.ExcelFile(self.excel_yolu)
            for s in dosya.sheet_names:
                temiz_sayfa = s.upper().strip()
                # Karışıklık olmaması için TAM EŞLEŞME yapıyoruz
                if temiz_sayfa == "ROTOR SACI":
                    gercek_sayfa = s
                    break
            # YENİ VE TEMİZ KISIM
        ...
        # --- 2. FORM ALANI (ROTOR PAKET ANA FORMUNUN BİREBİR ARAMA VERSİYONU) ---
        # --- 2. FORM ALANI (ROTOR SACI ANA FORMUNUN BİREBİR ARAMA VERSİYONU) ---
        grup_form = QGroupBox("Rotor Sacı Parametreleriyle Filtrele")
        ...
        self.grup_kutusu = QComboBox()
        # NORDEX ve 1 X 25MW eklendi!
        self.grup_kutusu.addItems(
            ["", "063", "071", "080", "090", "100", "112", "132", "160", "180", "200", "225", "250", "280", "315",
             "355", "400", "450", "500", "630", "NORDEX", "1 X 25MW"])

        self.kutup_kutusu = QComboBox()
        self.kutup_kutusu.addItems(["", "2", "4", "6", "8", "10"])

        # --- Rotor Sacı Detayları (PAKET BOYU SİLİNDİ) ---
        self.revizyon_kutusu = QComboBox()
    ...
    def arama_yap(self):
        try:
            ...
            # --- 1. DNA / Metin Havuzu Oluşturma ve TÜRKÇE KARAKTER TEMİZLİĞİ ---
            ...
            # --- YENİ SİLAH: SIFIR BOŞLUK DNA HAVUZU ---
            # Excel'deki "NOR" ve "DEX" veya "1X2" ve "5MW" parçalanmalarını birleştirir!
            ...
            if grup:
                if grup == "NORDEX":
                    # Excel'de "NOR" yazanları VEYA sıfır boşluk havuzunda "NORDEX" geçenleri yakala
                    df_filtrelenmis = df_filtrelenmis[
                        (df_filtrelenmis.iloc[:, 2].astype(str).str.strip().str.upper().isin(["NORDEX", "NOR"])) |
                        (metin_sifir_bosluk.str.contains("NORDEX", regex=False))
                        ]
                elif "25MW" in grup.replace(" ", ""):
                    # Excel'de "1X2" yazanları VEYA sıfır boşluk havuzunda "1X25MW" geçenleri yakala
                    df_filtrelenmis = df_filtrelenmis[
                        (df_filtrelenmis.iloc[:, 2].astype(str).str.strip().str.upper().isin([grup, "1X2"])) |
            ...
            # KURAL GÜNCELLEMESİ: Eski Gövde artık "-00" ile biten ID'leri tarıyor
            if versiyon == "ESKİ_GÖVDE":
                # ID kolonu Excel'de her zaman 0. indekstir. Son 2 hanesi "-00" olanları getirir.
                df_filtrelenmis = df_filtrelenmis[
            ...
            for index, row in df_filtrelenmis.iterrows():
                ...
                # Tablodaki sütun sayısını dinamik alarak taşmaları önlüyoruz
